...
Future<void> textToSpeechBot(String text) async {
  const String baseUrl = "http://server.ksprateek.studio"; // Replace with your base URL
  const String endpoint = "/api/speech/text-to-speech/";
  ...
  try {
    ...
    if (response.statusCode == 200) {
      if (response.headers['content-type'] == 'application/octet-stream') {
        ...
        // Play the file using DeviceFileSource (no need for isLocal)
        await audioPlayer.play(DeviceFileSource(filePath));
        await TranscriptionForChat.initialize() ;
        await TranscriptionForChat.startRecording(serverUrl);
        TranscriptionForChat.messages.listen((message) {
